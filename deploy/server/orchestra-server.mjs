...
const handleOrchestraApiRequest = async (req, res, targetBaseUrl) => {
  let targetUrl;
  try {
    targetUrl = new URL(targetBaseUrl);
  } catch {
    sendJson(res, 500, { error: 'Invalid ORCHESTRA_API_TARGET' });
    return;
  }

  if (targetUrl.protocol !== 'https:' && targetUrl.protocol !== 'http:') {
    sendJson(res, 500, { error: 'Unsupported ORCHESTRA_API_TARGET protocol' });
    return;
  }

  try {
    const upstreamUrl = new URL(req.url ?? '/', `${targetUrl.toString().replace(/\/+$/, '')}/`);
    const method = req.method ?? 'GET';
    const body = ['GET', 'HEAD'].includes(method) ? undefined : await readBody(req);

    if (isDebugEnabled()) {
      console.debug(`[Orchestra Proxy] ${method} ${req.url} -> ${upstreamUrl}`);
    }
    ...
    const message = error instanceof Error ? error.message : 'Unknown error';
    console.error('[Orchestra Proxy] Error:', message);
    sendJson(res, 502, { error: 'Orchestra API proxy error', message });
  }
};
...
export const createPrismaServer = ({ staticDir, allowedHosts } = {}) => {
  ...
  const orchestraApiTarget = process.env.ORCHESTRA_API_TARGET?.trim();

  return createServer(async (req, res) => {
    if (
      orchestraApiTarget &&
      (req.url === '/healthz' || req.url?.startsWith('/api/'))
    ) {
      await handleOrchestraApiRequest(req, res, orchestraApiTarget);
      return;
    }
