...
export function buildCreatorContext(input: {
  ...
  const workflow = snapshot?.stages.map((stage) => ({
    ...
    nodes: stage.nodes.map((node) => ({
      ...
      deliverableCount: node.product?.availableCount ?? 0,
  ...
  return {
    ...
    data: {
      ...
      summary: {
        ...
        workflow,
        stageProducts: selectedStage?.products ?? [],
        stageFiles: snapshot?.fileCatalog?.entries.filter((item) => item.stageId === selectedStage?.id) ?? [],
        fileCatalog: snapshot?.fileCatalog ? {
          root: snapshot.fileCatalog.root,
          counts: snapshot.fileCatalog.counts,
          total: snapshot.fileCatalog.entries.length,
        } : null,
        selectedNode: selectedNode ? {
          ...
          editorSession: selectedNode.editorSession ?? null,
          ...
          product: selectedNode.product,
          deliverables: selectedNode.product?.deliverables ?? [],
          supportingFiles: selectedNode.product?.supportingFiles ?? [],
          artifactLineage: selectedNode.artifacts,
    ...
    actions: [
      ...
      {
        id: "creator.editor.start-agent",
        label: "开始 Agent 协同剪辑",
        available: actionAvailable("creator.editor.start-agent"),
        inputSchema: {
          type: "object",
          required: ["editorId"],
          properties: {
      ...
      {
        id: "creator.editor.submit-proposal",
        label: "登记 Agent 剪辑提案",
        available: actionAvailable("creator.editor.submit-proposal"),
        inputSchema: {
          type: "object",
      ...
      {
        id: "creator.editor.review-proposal",
        label: "同步剪辑提案审核结果",
        available: actionAvailable("creator.editor.review-proposal"),
        inputSchema: {
          type: "object",
          required: ["decision"],
          properties: {
            ...
            summary: { type: "string" },
            changeCount: { type: "integer", minimum: 0 },
            decision: { type: "string", enum: ["applied", "rejected", "discarded"] },
            note: { type: "string" },
          },
        },
      },
      {
        id: "creator.editor.import-export",
        label: "导入 OpenChatCut 导出",
        available: actionAvailable("creator.editor.import-export"),
        inputSchema: {
          type: "object",
      ...
      {
        id: "creator.editor.save-template",
        label: "把已跑通剪辑保存为模板",
        available: actionAvailable("creator.editor.save-template"),
        inputSchema: {
          type: "object",
          ...
          properties: {
            ...
            name: { type: "string" },
            mode: { type: "string", enum: ["project", "selection"] },
            sourceAction: { type: "string", const: "manage_template.save" },
          },
        },
      },
      {
