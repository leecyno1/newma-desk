...
宏观观察与 Circle / 周期叠加的边界如下：

- 宏观观察负责可核验的指标、发布日历、前值、预期、实际值和数据修订风险；
- 周期叠加负责通过发布门槛后的概率观察窗；
- 周期叠加概率不得反写宏观指标，也不得被解释为确定性的经济拐点或精确日期。
...
宏观指标优先读取 Tushare Pro 结构化宏观序列和经济日历，缺失或失效时自动降级到 AkShare 的金十、东方财富等公开聚合数据。生产环境配置 `TUSHARE_TOKEN` 后启用主链路。聚合源不是官方原始发布机构，因此合同固定输出 `official-primary-source-verification` 缺口，Agent 和用户应在形成重要判断前回到国家统计局、人民银行、FRED、央行或对应统计机构复核。
