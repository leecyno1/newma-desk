...
VibeDesk 是一个面向人和 Agent 的可生长工作台。用户可以把临时需求固化成长期可使用、可更新、可继续迭代的 Mod。
...
| 产品词汇 | 定义 | 代码名称 |
| --- | --- | --- |
| VibeDesk | 产品整体 | `vibedesk` |
| Desk | 默认前端、侧边栏和运行容器 | `@vibedesk/desk` |
| Mod | 可安装、发布和升级的功能单元 | `ModManifest` |
| View | Mod 中可独立访问的 HTML 页面 | `View` |
| Skill | Agent 可重复执行的工作流和技巧 | `Skill` |
| Desk UI | 统一视觉变量和基础组件 | `@vibedesk/desk-ui` |
| ViewSpec | 面向人和 Agent 的 HTML 语义规范 | `ViewSchema` |
| Mod SDK | 数据、AI、导航和跨 Mod 通信入口 | `@vibedesk/mod-sdk` |
...
它们通过适配层向 VibeDesk 提供数据或页面，业务后端继续独立部署和独立升级。
