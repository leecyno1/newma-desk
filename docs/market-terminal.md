...
市场终端 Mod（OpenTerminalUI 风格的高密度交互）
        ↑
Newma-Desk Data Service + Bridge + Mod Context + Desk Copilot
```
...
- 提问和修改统一使用 Newma-Desk 右侧 Desk Copilot；市场终端内部不再保留重复的 Agent、模型选择和回答区。
...
- 市场只负责股票、债券及其他交易市场的数据、图表、扫描和回放，不承载全球情报页面。
- 全球态势、新闻、政策、冲突、灾害、军事与关键基础设施统一进入首页“情报 → 全球情报”。

## Chart Kit 与第一批图表 Mods

通用图表能力已经抽取到 `@newma-desk/chart-kit`。市场终端和新图表 Mods 共用同一套 KLineChart 生命周期、主题、指标、画线接口和相对强弱图表，不再各自维护图表初始化代码。

市场图表工作区以独立 Level 3 Mods 发布，并共享 `market-daily` 运行时：
...
| 新闻与舆情 | 归入情报 | 独立承担新闻流、来源、主题追踪和舆情，与全球情报中的政策、冲突和灾害分类互补。 |
...
`market-data` 的描述文件保留稳定能力 ID，实际服务地址由 `NEWMA_DESK_RESEARCH_BASE_URL` 覆盖。这样本地进程内 Research Suite、测试假服务和未来独立数据服务可以复用同一前端协议，避免把端口写死在 Mod 中。

Research 与 Trading 当前作为 `mod-projects/` 下的内置领域源码，由 Newma-Desk API 进程内挂载。它们保留独立 Git 历史用于上游同步，但标准用户不需要单独启动两个服务。
