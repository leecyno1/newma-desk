...
function requireOwnedEditSession(
  ...
  allowUnownedDiscard: boolean,
): void {
  if (editSessionOwnerMatches(ownerId, binding, args.editSessionId)) return;
  const sessionId = typeof args.editSessionId === 'string'
    ? args.editSessionId.trim()
    : '';
  if (allowUnownedDiscard && sessionId && !editSessionOwners.has(sessionId)) return;
...
export function invokeEditorTool(
  ...
  if (ownsSession) {
    requireOwnedEditSession(ownerId, binding, args, name === 'discard_edit_session');
  }
...
export function cancelEditorCallsForOwner(
  ...
  for (const [sessionId, owner] of editSessionOwners) {
    ...
    const currentEditor = editorBinding(owner.binding.projectId) ?? owner.binding;
    const key = editorKey(currentEditor.projectId, currentEditor.editorInstanceId);
