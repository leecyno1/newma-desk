...
const handoffProject = 'project-owner-gone-handoff';
const oldEditor = 'editor-before-owner-gone';
const newEditor = 'editor-after-owner-gone';
const oldCapability = registerEditor(
  handoffProject,
  oldEditor,
  'v1-owner-gone',
  tools,
...
const oldBinding = editorBinding(handoffProject);
assert(oldBinding);
const beginPromise = invokeEditorTool(
  'owner-gone-transport',
  oldBinding,
  'begin_edit_session',
  {},
);
const beginCall = await nextEditorCall(
  handoffProject,
  oldEditor,
  oldBinding.baseRevision,
  AbortSignal.timeout(100),
  oldCapability,
);
assert(beginCall);
settleEditorCall(beginCall.id, 'applied', { editSessionId: 'session-owner-gone' }, oldCapability);
await beginPromise;
await unregisterEditor(handoffProject, oldEditor, oldCapability);
const newCapability = registerEditor(
  handoffProject,
  newEditor,
  'v2-owner-gone',
  tools,
...
cancelEditorCallsForOwner('owner-gone-transport');
const ownerGone = await nextEditorCancellation(
  handoffProject,
  newEditor,
  AbortSignal.timeout(100),
  newCapability,
...
assert.deepEqual(
  ownerGone?.ownerGone,
  ['session-owner-gone'],
  'orphaned sessions follow the project to the replacement editor instance',
);
const replacementBinding = editorBinding(handoffProject);
assert(replacementBinding);
const orphanDiscardPromise = invokeEditorTool(
  'owner-after-broker-restart',
  replacementBinding,
  'discard_edit_session',
  { editSessionId: 'session-owner-gone' },
);
const orphanDiscardCall = await nextEditorCall(
  handoffProject,
  newEditor,
  replacementBinding.baseRevision,
  AbortSignal.timeout(100),
  newCapability,
);
assert.equal(
  orphanDiscardCall?.name,
  'discard_edit_session',
  'an unowned draft can be cancelled after broker ownership memory is lost',
);
settleEditorCall(
  orphanDiscardCall!.id,
  'applied',
  { editSessionId: 'session-owner-gone', status: 'cancelled' },
  newCapability,
);
await orphanDiscardPromise;
