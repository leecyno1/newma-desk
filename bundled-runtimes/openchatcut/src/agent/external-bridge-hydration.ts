...
interface HydratedExternalSession {
  ...
  readonly pendingApprovals?: readonly ExternalApprovalBinding[];
}
...
async function hydrateDrafting(input: ExternalBridgeHydrationInput): Promise<void> {
  ...
  const pendingApprovals = await run.pendingApprovals(session.id);
  input.install({ session, run, pendingApprovals });
