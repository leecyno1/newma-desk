...
function externalApprovalCancellationRunIds(external: StoredExternalProposal | null): Set<string> {
  return external?.status === 'awaiting_review' && external.agentRunId
    ? new Set([external.agentRunId])
    : new Set();
}
...
export async function loadRecoveredAgentSession(
  ...
  const approvalCancellationRunIds = externalApprovalCancellationRunIds(external);
  ...
  const needsRecovery = sidecarBefore.runs.some((run) => {
    ...
    if (preservedRunIds.has(run.runId)) {
      return approvalCancellationRunIds.has(run.runId)
