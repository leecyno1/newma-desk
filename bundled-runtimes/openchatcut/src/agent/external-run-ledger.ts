...
export class ExternalSessionRunLedger {
  ...
  static async resume(
    ...
    const recorder = await resumeAgentRun(
      projectId,
      runId,
      claimedLeaseToken,
      replaceActive,
  ...
  async pendingApprovals(sessionId: string): Promise<ExternalApprovalBinding[]> {
    const sidecar = await loadAgentRuntimeSidecar(this.recorder.projectId);
    const run = sidecar.runs.find((item) => item.runId === this.runId);
    if (run?.externalSessionId !== sessionId) return [];
    return sidecar.approvals
      .filter((approval) => approval.runId === this.runId && approval.status === 'pending')
      .map((approval) => ({
        guardId: approval.approvalId,
        sessionId,
        ...
        summary: approval.summary ?? approval.toolName,
        details: [],
      }));
  }
