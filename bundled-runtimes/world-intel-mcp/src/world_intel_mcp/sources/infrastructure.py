...
_IODA_OUTAGES_URL = "https://api.ioda.inetintel.cc.gatech.edu/v2/outages/alerts"
...
async def _fetch_ioda_outages(fetcher: Fetcher) -> dict | None:
    ...
    # IODA v2 alerts are flat records with an embedded entity.
    ...
    for item in raw_items:
        ...
        entity_name = str(entity.get("name") or attrs.get("name") or entity_code)
        datasource = str(item.get("datasource") or "ioda")
        event_time = item.get("time")

        country_code = str(attrs.get("country_code") or "").upper()
        if not country_code and entity_type == "country" and len(entity_code) == 2:
            country_code = entity_code.upper()
        if not country_code and entity_type == "geoasn":
            suffix = entity_code.rsplit("-", 1)[-1].upper()
            if len(suffix) == 2 and suffix.isalpha():
                country_code = suffix

        asns: list[str] = []
        if entity_type in {"asn", "geoasn"}:
            asn = entity_code.split("-", 1)[0]
            if asn.isdigit():
                asns.append(f"AS{asn}")

        start = event_time
        if isinstance(event_time, (int, float)):
            start = datetime.fromtimestamp(event_time, timezone.utc).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            )
        ...
        comparison = ""
        if value is not None and history_value is not None:
            comparison = f" ({value} vs {history_value} historical)"

        outages.append({
            "id": f"{datasource}:{entity_type}:{entity_code}:{event_time}",
            "start": start,
            "end": None,
            "description": f"{level.title()} {datasource} outage alert for {entity_name}{comparison}",
            "scope": entity_type,
            "severity": level,
            "entity": entity_name,
