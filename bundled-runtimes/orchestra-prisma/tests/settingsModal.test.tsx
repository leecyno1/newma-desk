...
describe('SettingsModal', () => {
    ...
    fetchMock.mockImplementation((url: string) => {
      if (url.endsWith('/releases/latest')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ tag_name: currentVersion }),
        });
      }
  ...
  it('renders the about panel in an AMC-style centered presentation', async () => {
    ...
    const releaseLink = screen.getByRole('link', { name: currentVersionPattern });
  ...
  it('localizes the release status when a newer Prisma release is available', async () => {
    ...
    fetchMock.mockImplementation((url: string) => {
      if (url.endsWith('/releases/latest')) {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve({ tag_name: newerVersion }),
        });
      }
    ...
    await waitFor(() => {
      ...
      expect(releaseLink.getAttribute('title')).toBe(`有新版本：${newerVersion}`);
    });
  });
});
