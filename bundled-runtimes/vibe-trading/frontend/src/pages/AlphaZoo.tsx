...
import { createVibeDeskSnapshotCache, subscribeVibeDeskConfig } from "@/lib/vibedesk";
...
const PAGE_SIZE = 50;
const INTEGRATED_DESK_RUNTIME = import.meta.env.VITE_NEWMA_DESK_INTEGRATED === "1";
const JOB_POLL_INTERVAL_MS = 750;
const AlphaThemeChart = lazy(() =>
  import("@/components/charts/AlphaThemeChart").then((module) => ({
    default: module.AlphaThemeChart,
  })),
);
...
function BrowseView() {
  ...
  const catalogResourceKey = `alpha-zoo:catalog:${zooFilter || "all"}:${themeFilter || "all"}:${universeFilter || "all"}`;
  const catalogCacheKey = `${catalogResourceKey}:${cacheRevision}`;
  const browseCache = useMemo(() => createVibeDeskSnapshotCache<{
    alphas: AlphaSummary[];
    total: number;
  }>(catalogResourceKey, 1, 1024 * 1024), [cacheRevision, catalogResourceKey]);
  const cachedBrowse = useMemo(() => browseCache.read()?.value, [browseCache]);
  const [alphas, setAlphas] = useState<AlphaSummary[]>(() => cachedBrowse?.alphas ?? []);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [visibleCount, setVisibleCount] = useState(PAGE_SIZE);
  const [total, setTotal] = useState<number>(() => cachedBrowse?.total ?? 0);
  // Alphas ticked for a head-to-head compare; handed to CompareView via the URL.
  const [selected, setSelected] = useState<Set<string>>(() => new Set());
  const catalogResourceKeyRef = useRef<string | undefined>(undefined);
  ...
  useEffect(() => {
    let alive = true;
    setLoading(true);
    const cached = browseCache.read()?.value;
    const resourceChanged = catalogResourceKeyRef.current !== catalogCacheKey;
    catalogResourceKeyRef.current = catalogCacheKey;
    if (resourceChanged) {
      setAlphas(cached?.alphas ?? []);
    ...
    api
      ...
      .then((res) => {
        ...
        setTotal(res.total);
        browseCache.write({ alphas: res.alphas, total: res.total });
        setVisibleCount(PAGE_SIZE);
      })
      .catch((err: unknown) => {
        if (!alive) return;
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      {/* Hero */}
      <div className="space-y-2" data-mod-page-title>
...
function DetailView({ alphaId }: DetailProps) {
  const [cacheRevision, setCacheRevision] = useState(0);
  const detailCache = useMemo(() => createVibeDeskSnapshotCache<AlphaDetailResponse>(
    `alpha-zoo:detail:${alphaId}`,
  ), [alphaId, cacheRevision]);
  const [detail, setDetail] = useState<AlphaDetailResponse | null>(() => detailCache.read()?.value ?? null);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    setDetail(detailCache.read()?.value ?? null);
    api
      .getAlpha(alphaId)
      .then((res) => {
        if (alive) {
          setDetail(res);
          detailCache.write(res);
        }
      })
      .catch((err: unknown) => {
        if (!alive) return;
        ...
        setError(msg);
        if (detailRef.current) toast.error(`${msg}. Showing the last saved detail.`);
      })
      .finally(() => {
        if (alive) setLoading(false);
      });
...
function BenchView() {
  ...
  const [oosSplit, setOosSplit] = useState("");
  ...
  const startBench = async (e: FormEvent) => {
    ...
    try {
      const res = await api.createAlphaBench({
        ...
        top: safeTop,
        strict,
        oos_split: strict && oosSplit ? oosSplit : null,
        random_seeds: 5,
      });
      setJobId(res.job_id);
  ...
  const pollBench = async (newJobId: string, version: number) => {
    try {
      const job = await api.getAlphaBench(newJobId);
      ...
      if (job.status === "error") {
        toast.error(job.error || "Bench failed");
        setStatus("error");
        return;
      }
      pollTimerRef.current = window.setTimeout(
        () => void pollBench(newJobId, version),
        JOB_POLL_INTERVAL_MS,
      );
    } catch (err) {
      if (pollVersionRef.current !== version) return;
      toast.error(err instanceof Error ? err.message : "Bench status unavailable");
      setStatus("error");
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-1" data-mod-page-title>
        <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-wide">
      ...
      <form
        onSubmit={startBench}
        className="border rounded-xl p-4 bg-card grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end"
      >
        <div>
        ...
        <div>
          <label htmlFor="bench-oos-split" className="text-xs text-muted-foreground block mb-1">
            OOS split
          </label>
          <input
            id="bench-oos-split"
            type="date"
            value={oosSplit}
            onChange={(e) => setOosSplit(e.target.value)}
            disabled={busy || !strict}
            className="w-full px-3 py-2 rounded-lg border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50"
          />
        </div>
...
function ResultPanel({ result }: { result: AlphaBenchResult }) {
  const strict = result.random_control === true;
  const totals = [
    { label: strict ? "Confirmed" : i18n.t("alphaZoo.alive"), value: strict ? result.confirmed_alive ?? 0 : result.alive, icon: CheckCircle2, tone: "text-green-600 dark:text-green-400" },
    { label: strict ? "Train only" : i18n.t("alphaZoo.reversed"), value: strict ? result.train_only ?? 0 : result.reversed, icon: AlertTriangle, tone: "text-amber-600 dark:text-amber-400" },
    { label: strict ? "Reversed" : i18n.t("alphaZoo.dead"), value: strict ? result.reversed_strict ?? 0 : result.dead, icon: XCircle, tone: "text-red-600 dark:text-red-400" },
    ...(strict ? [{ label: "Noise", value: result.noise ?? 0, icon: Loader2, tone: "text-muted-foreground" }] : []),
  ...
  return (
    <div className="space-y-4">
      ...
      {result.meta && (
        <div className="border rounded-xl px-4 py-3 bg-card flex flex-wrap gap-x-5 gap-y-1 text-xs text-muted-foreground">
          <span>{result.meta.universe}</span>
          <span>{result.meta.constituent_source}</span>
          <span>{result.meta.constituent_count ?? 0} constituents</span>
          {result.meta.constituent_source_date && <span>{result.meta.constituent_source_date}</span>}
          {result.meta.survivorship_bias && <span>Survivorship biased</span>}
          {result.meta.degraded && <span className="text-amber-600 dark:text-amber-400">Degraded universe</span>}
        </div>
      )}
      ...
      {result.by_theme && Object.keys(result.by_theme).length > 0 && (
        <div className="border rounded-xl p-4 bg-card">
          ...
          <Suspense fallback={<SkeletonChart height={240} />}>
            <AlphaThemeChart byTheme={result.by_theme} height={240} />
          </Suspense>
        </div>
      )}
    </div>
...
function CategoryBadge({ category }: { category: AlphaBenchTopRow["category"] }) {
  const confirmed = category === "alive" || category === "confirmed_alive";
  const reversed = category === "reversed" || category === "reversed_strict";
  const tone =
    confirmed
      ? "bg-green-500/10 text-green-700 dark:text-green-300"
      : reversed || category === "train_only"
...
function CompareView() {
  ...
  const pollCompare = async (newJobId: string, version: number) => {
    try {
      const job = await api.getAlphaCompare(newJobId);
      if (pollVersionRef.current !== version) return;
      setProgress(job.progress);
      if (job.status === "done") {
        setResult(job.result ?? null);
      ...
      if (job.status === "error") {
        toast.error(job.error || "Comparison failed");
        setStatus("error");
        return;
      }
      pollTimerRef.current = window.setTimeout(
        () => void pollCompare(newJobId, version),
        JOB_POLL_INTERVAL_MS,
      );
    } catch (err) {
      if (pollVersionRef.current !== version) return;
      toast.error(err instanceof Error ? err.message : "Comparison status unavailable");
      setStatus("error");
    }
  };
