...
interface SettingsPageShellProps {
  children: ReactNode;
  isLoaded: boolean;
  localApiAccessSection: ReactNode;
}
...
function SettingsPageShell({
  ...
  const showEmbeddedCopy = isVibeDeskEmbedded && isLoaded;

  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <div className="space-y-2" data-mod-page-title>
        <h1 className="text-2xl font-semibold tracking-tight">
          {showEmbeddedCopy ? "量化系统设置" : "Settings"}
        </h1>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {showEmbeddedCopy
            ? "配置量化数据源、运行通道和领域服务；Agent 由 VibeDesk 统一管理。"
            : "Configure model credentials and market data source tokens for this local project."}
        </p>
      </div>

      {!isVibeDeskEmbedded ? localApiAccessSection : isLoaded ? (
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <Server className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
            <div className="space-y-1">
              <h2 className="text-base font-semibold">Agent 由 VibeDesk 统一提供</h2>
              <p className="text-sm text-muted-foreground">
                模型、CLI 和长期上下文请在 VibeDesk 的“Agent 设置”中管理；本页仅保留量化系统自身的数据源与运行配置。
              </p>
            </div>
          </div>
        </section>
...
export function Settings() {
  ...
  useEffect(() => {
    ...
    if (isVibeDeskEmbedded) {
      api.getDataSourceSettings()
        .then((value) => {
          if (alive) setDataSettings(value);
        })
        .catch((error) => {
          if (!alive) return;
  ...
  if (
    loading ||
    !dataSettings ||
    (!isVibeDeskEmbedded && (!form || !settings))
  ) {
    return (
      <SettingsPageShell
        isLoaded={false}
        localApiAccessSection={localApiAccessSection}
      >
  ...
  const modelForm: LLMFormState = form ?? {
    provider: "",
    model_name: "",
    base_url: "",
    temperature: 0,
  ...
  return (
    <SettingsPageShell
      ...
      localApiAccessSection={localApiAccessSection}
    >
      {!isVibeDeskEmbedded ? channelsSection : null}
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(event) => setForm({ ...modelForm, model_name: event.target.value })}
                  className={fieldClass}
                  required
                />
                <button
            ...
            <label className="grid gap-2">
              ...
              <input
                ...
                onChange={(event) => setForm({ ...modelForm, base_url: event.target.value })}
                className={fieldClass}
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <input
                ...
                onChange={(event) => setForm({ ...modelForm, temperature: Number(event.target.value) })}
                className={fieldClass}
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <input
                ...
                onChange={(event) => setForm({ ...modelForm, timeout_seconds: Number(event.target.value) })}
                className={fieldClass}
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <input
                ...
                max={20}
                step={1}
                value={modelForm.max_retries}
                onChange={(event) => setForm({ ...modelForm, max_retries: Number(event.target.value) })}
                className={fieldClass}
              />
            </label>
            ...
            <label className="grid gap-2">
              ...
              <select
                ...
                onChange={(event) => setForm({ ...modelForm, reasoning_effort: event.target.value })}
                className={fieldClass}
              >
