...
const EquityChart = lazy(() =>
  import("@/components/charts/EquityChart").then((module) => ({
    default: module.EquityChart,
  })),
);
...
type RunDetailSnapshot = { run: RunData; code: Record<string, string> };

function isCompletedRun(run: RunData | null): run is RunData {
  return Boolean(run) && !["queued", "pending", "running", "in_progress"].includes(
...
export function RunDetail() {
  ...
  const loadChartSymbolRef = useRef<(symbol: string) => Promise<void>>(async () => {});
  const runRef = useRef(run);
  runRef.current = run;
  const [cacheRevision, setCacheRevision] = useState(0);
  const detailCache = useMemo(
    () => createVibeDeskSnapshotCache<RunDetailSnapshot>(`run-detail:${runId || "missing"}`, 1, 2 * 1024 * 1024),
    [cacheRevision, runId],
  );
  const detailResourceRef = useRef("");
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    const cached = detailCache.read()?.value;
    const resource = `${cacheRevision}:${runId}`;
    const resourceChanged = detailResourceRef.current !== resource;
    detailResourceRef.current = resource;
    const applyRun = (nextRun: RunData | null, nextCode: Record<string, string>) => {
      if (!active) return;
      setRun(nextRun);
      setCode(nextCode);
      const firstSymbol = nextRun?.chart_symbols?.[0] || Object.keys(nextRun?.price_series || {})[0] || "";
      setSelectedSymbol(firstSymbol);
      setChartPickerSymbol(firstSymbol);
      setSelectedSymbols(firstSymbol ? [firstSymbol] : []);
      const initialCache = cacheFromRun(nextRun, firstSymbol);
      ...
      if (firstSymbol && !initialCache[firstSymbol]?.price_series?.[firstSymbol]?.length) {
        void loadChartSymbolRef.current(firstSymbol);
      }
    };
    if (resourceChanged) applyRun(cached?.run ?? null, cached?.code ?? {});
    else if (cached && !runRef.current) applyRun(cached.run, cached.code);
    Promise.all([
      api.getRun(runId, { chart_payload: "summary" }).catch(() => null),
      api.getRunCode(runId).catch(() => cached?.code ?? {}),
    ]).then(([r, c]) => {
      if (!active) return;
      if (r) {
        const nextCode = c || {};
        applyRun(r, nextCode);
        if (isCompletedRun(r)) detailCache.write({ run: r, code: nextCode });
      }
    }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [detailCache, runId]);
  useEffect(() => subscribeVibeDeskConfig(() => setCacheRevision((value) => value + 1)), []);
  loadChartSymbolRef.current = loadChartSymbol;
    ...
    <div className="p-4 space-y-4">
      ...
      {entries.map(([sym, bars]) => (
        <div key={sym}>
          ...
          <Suspense fallback={<SkeletonChart height={500} />}>
