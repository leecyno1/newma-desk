import { requestAuthHeaders, withAuthQuery } from "@/lib/apiAuth";
import { domainApi } from "@/lib/domainApi";
import { apiUrl, errorFromResponse, request } from "@/lib/apiClient";
...
export type QuickRunTemplateId = "buy_and_hold" | "sma_crossover";
...
export interface QuickRunRequest {
  template_id: QuickRunTemplateId;
  symbol: string;
  start_date: string;
  end_date: string;
  params?: QuickRunParams;
}
...
export interface QuickRunStatusResponse {
  run_id: string;
  status: string;
}
...
export const api = {
  ...
  sseUrl: (sid: string, options?: { replay?: "active" }) => {
    let url = withAuthQuery(apiUrl(`/sessions/${sid}/events`));
...
export interface RunListItem {
  ...
  start_date?: string;
  end_date?: string;
  strategy_ledger?: StrategyLedgerRecord;
}

export interface StrategyLedgerExperiment {
  id: string;
  revision: string;
...
export interface StrategyLedgerMetrics {
...
export interface StrategyLedgerAttributionItem {
  kind: "return" | "risk" | "cost" | "activity";
  factor: string;
  value: number;
  unit: "ratio" | "rate" | "currency" | "count" | string;
  note?: string;
}

export interface StrategyLedgerRecord {
  schema_version: "newma-desk.strategy-ledger.v1";
  ledger_id: string;
  experiment: StrategyLedgerExperiment;
  mode: "paper-only";
  ...
  metrics: StrategyLedgerMetrics;
  attribution: StrategyLedgerAttributionItem[];
  cost_model: {
    commission_rate?: number | null;
    realized_fees_available: boolean;
  };
  quality: {
    level: "complete" | "usable" | "limited" | "pending";
    flags: string[];
  };
...
export interface AlphaBenchResult {
  ...
  random_control?: boolean;
  n_random_seeds?: number;
  oos_split?: string | null;
  confirmed_alive?: number;
  train_only?: number;
  reversed_strict?: number;
  noise?: number;
  meta?: {
    universe?: string;
    constituent_source?: string;
    constituent_source_date?: string | null;
...
export interface AlphaBenchJob {
  job_id: string;
...
export interface AlphaCompareJob {
  job_id: string;
  status: "queued" | "running" | "done" | "error";
  progress: {
    n_done: number;
  ...
  result?: AlphaCompareResult | null;
  error?: string | null;
}
