...
export function CandlestickChart({ data, markers, indicators, height = 500 }: Props) {
  ...
  }, [data.length === 0, dark, themeRevision]); // re-init when data availability or Desk appearance changes
  ...
  useEffect(() => {
    ...
    for (const [key, { name, data: lineData }] of Object.entries(overlayMap)) {
      if (overlays.has(key as Overlay)) {
        overlaySeries.push({ name, type: "line", data: lineData, xAxisIndex: 0, yAxisIndex: 0, symbol: "none", lineStyle: { color: t.seriesColors[colorIdx % t.seriesColors.length], width: 1 } });
    ...
    const marks = (markers || []).map(m => ({
      ...
      label: { color: t.markerText, fontSize: 10, fontWeight: "bold" as const },
    }));
    ...
    if (sub === "vol") {
      ...
      subSeries = [
        ...
        { name: "%J", type: "line", data: kdj.j, xAxisIndex: 1, yAxisIndex: 1, symbol: "none", lineStyle: { width: 1, color: t.seriesColors[2] } },
      ];
    ...
    const extraSeries = extraIndicators.map((ind, i) => {
      ...
      return { name: ind.name, type: "line" as const, data: ind.values, xAxisIndex: 0, yAxisIndex: 0, symbol: "none", lineStyle: { width: 1, color: t.seriesColors[(colorIdx + i) % t.seriesColors.length], type: "dashed" as const } };
    });
  ...
  }, [data, markers, baseData, indicatorCache, extraIndicators, sub, range, overlays, dark, themeRevision]);
