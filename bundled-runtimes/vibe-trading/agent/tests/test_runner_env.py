...
def test_backtest_python_candidates_include_workspace_venv_before_host_python() -> None:
    candidates = Runner(timeout=1)._python_candidates()

    assert candidates[1].as_posix().endswith("/vibe-trading/.venv/bin/python")
...
def test_python_probe_ignores_macos_virtualenv_launcher(monkeypatch, tmp_path: Path) -> None:
    """A launcher from the API venv must not redirect the Trading probe."""

    monkeypatch.setenv(
        "__PYVENV_LAUNCHER__",
        "/Volumes/PSSD/Projects/newma-desk/services/api/.venv/bin/python",
    )
    trading_python = Path(__file__).resolve().parents[2] / ".venv" / "bin" / "python"

    assert trading_python.exists()
    assert Runner(timeout=1)._python_ready(str(trading_python)) is True
