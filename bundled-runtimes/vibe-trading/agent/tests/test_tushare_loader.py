...
fund_daily(), indices need index_daily(), HK needs hk_daily(), and US needs
us_daily(). Crypto remains unsupported.
"""
...
class TestIsUsEquity:
    ...
    def test_tushare_symbol_conversion(self) -> None:
        assert _to_tushare_us_code("AAPL.US") == "AAPL"
        assert _to_tushare_hk_code("700.HK") == "00700.HK"
        assert _to_tushare_hk_code("00700.HK") == "00700.HK"
...
class TestFetchDailyFrameRouting:
    ...
    def test_hk_routes_to_hk_daily(self) -> None:
        ...
        result = loader._fetch_daily_frame("700.HK", "20250102", "20250110")
        loader.api.hk_daily.assert_called_once_with(
            ts_code="00700.HK", start_date="20250102", end_date="20250110",
        )
        loader.api.daily.assert_not_called()
        assert result is not None

    def test_us_routes_to_us_daily(self) -> None:
        loader = self._make_loader()
        loader.api.us_daily.return_value = _make_ohlcv_df()
        result = loader._fetch_daily_frame("AAPL.US", "20250102", "20250110")
        loader.api.us_daily.assert_called_once_with(
            ts_code="AAPL", start_date="20250102", end_date="20250110",
        )
        loader.api.daily.assert_not_called()
        loader.api.fund_daily.assert_not_called()
        assert result is not None
    ...
    def test_fetch_batches_same_endpoint_symbols(self, monkeypatch) -> None:
        monkeypatch.setenv("VIBE_TRADING_DATA_CACHE", "0")
        monkeypatch.setenv("TUSHARE_BATCH_SIZE", "10")
        loader = self._make_loader()
        loader.api.daily.return_value = pd.DataFrame(
            {
        ...
        loader.api.daily.assert_called_once_with(
            ts_code="000001.SZ,600000.SH",
            start_date="20240101",
            end_date="20240105",
        )
        assert set(data) == {"000001.SZ", "600000.SH"}
...
class TestTushareE2E:
    ...
    def _fetch(self, codes: list[str]) -> dict[str, pd.DataFrame]:
        ...
        # Other provider/config tests mutate the process environment. Pin this
        # E2E call to the token captured during collection so test order cannot
        # replace a valid credential with a fixture placeholder.
        with patch.dict(os.environ, {"TUSHARE_TOKEN": _token}):
            reset_env_config()
            try:
                loader = DataLoader()
