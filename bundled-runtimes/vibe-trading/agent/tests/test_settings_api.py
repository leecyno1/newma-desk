...
def test_integrated_settings_surface_excludes_desk_owned_llm_routes() -> None:
    integrated_app = FastAPI()

    register_settings_routes(
        integrated_app,
        require_local_or_auth=lambda: None,
        require_settings_write_auth=lambda: None,
        include_llm=False,
    )

    paths = {route.path for route in integrated_app.routes}
    assert "/settings/data-sources" in paths
    assert "/settings/llm" not in paths
