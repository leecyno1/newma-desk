...
from src.providers.chat import (
    ...
    _should_disable_streaming_tool_calls,
)
...
class TestSiliconFlowToolStreamingGuard:
    def test_detects_siliconflow_openai_compat(self, monkeypatch) -> None:
        monkeypatch.setenv("LANGCHAIN_PROVIDER", "openai")
        monkeypatch.setenv("OPENAI_BASE_URL", "https://api.siliconflow.cn/v1")
        monkeypatch.delenv("VIBE_TRADING_STREAM_TOOL_CALLS", raising=False)

        assert _should_disable_streaming_tool_calls() is True

    def test_override_keeps_streaming_enabled(self, monkeypatch) -> None:
        monkeypatch.setenv("OPENAI_BASE_URL", "https://api.siliconflow.cn/v1")
        monkeypatch.setenv("VIBE_TRADING_STREAM_TOOL_CALLS", "1")

        assert _should_disable_streaming_tool_calls() is False

    def test_stream_chat_delegates_to_chat_when_guarded(self, monkeypatch) -> None:
        monkeypatch.setenv("OPENAI_BASE_URL", "https://api.siliconflow.cn/v1")
        monkeypatch.delenv("VIBE_TRADING_STREAM_TOOL_CALLS", raising=False)
