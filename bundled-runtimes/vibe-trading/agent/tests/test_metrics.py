...
class TestBarsPerYear:
    ...
    def test_every_registered_source_has_intraday_annualisation(self) -> None:
        from backtest.loaders.registry import VALID_SOURCES

        for source in VALID_SOURCES - {"auto"}:
            assert calc_bars_per_year("1m", source) > 252

    def test_interval_and_source_are_case_insensitive(self) -> None:
        assert calc_bars_per_year("1h", "Yahoo") == 252 * 7
        assert calc_bars_per_year("1D", "OKX") == 365


def test_bar_returns_preserves_move_across_gap() -> None:
    close = pd.Series([100.0, 110.0, np.nan, np.nan, 121.0])

    result = bar_returns(close)

    assert result.tolist() == pytest.approx([0.0, 0.1, 0.0, 0.0, 0.1])


def test_bar_returns_never_emits_non_finite_values() -> None:
    result = bar_returns(pd.Series([10.0, 0.0, 42.0, np.inf, 50.0]))
