...
class TestPositionResize:
    @staticmethod
    def _engine() -> ChinaAEngine:
        return ChinaAEngine(
            {
    ...
    def test_same_direction_rebalance_reduces_and_then_increases(self) -> None:
        engine = self._engine()
        ...
        engine._rebalance("000001.SZ", 0.5, frame, dates[0], 100_000)
        assert engine.positions["000001.SZ"].size == 5_000

        engine._bar_idx = 1
        engine._rebalance("000001.SZ", 0.2, frame, dates[1], 100_000)
        assert engine.positions["000001.SZ"].size == 2_000
        assert len(engine.trades) == 1
        assert engine.trades[0].size == 3_000
        assert engine.trades[0].exit_reason == "rebalance"

        engine._bar_idx = 2
        engine._rebalance("000001.SZ", 0.4, frame, dates[2], 100_000)
        assert engine.positions["000001.SZ"].size == 4_000
        assert len(engine.trades) == 1
