...
def test_compare_rejects_single_asset_crypto_universe() -> None:
    r = _client().post("/alpha/compare", json=_valid_body(universe="btc-usdt"))
    assert r.status_code == 422
...
def test_authenticated_status_snapshot_uses_safe_wire_projection() -> None:
    _seed_job(
        "status4444",
        status="done",
    ...
    response = _client().get("/alpha/compare/status4444")
    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "done"
    assert payload["result"]["winner"] == "alpha101_2"
    assert "status" not in payload["result"]


def test_status_snapshot_rejects_unknown_job() -> None:
    response = _client().get("/alpha/compare/missing4444")
