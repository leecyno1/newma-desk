...
class TestSymbolGating:
    ...
    def test_accepts_us(self):
        ...
        assert _is_supported("aapl.us") is True
        assert _is_supported("AAPL") is True
        assert _is_supported("BRK.B") is True

    def test_accepts_hk(self):
        assert _is_supported("00700.HK") is True
        assert _is_supported("00700.hk") is True
        assert _is_supported("00700") is True
...
class TestFetch:
    ...
    def test_fetch_bare_us_symbol_preserves_key(self):
        rows = [_row("2024-01-02", 10, 11, 9, 10.5, 1000)]
        with patch(
            "backtest.loaders.yahoo_loader.yahoo_client.get_chart",
            return_value=rows,
        ) as mock_chart:
            out = DataLoader().fetch(["AAPL"], "2024-01-01", "2024-01-31")

        assert set(out) == {"AAPL"}
        assert mock_chart.call_args.args[0] == "AAPL"

    def test_fetch_bare_hk_symbol_adds_market_for_yahoo(self):
        rows = [_row("2024-01-02", 10, 11, 9, 10.5, 1000)]
        with patch(
            "backtest.loaders.yahoo_loader.yahoo_client.get_chart",
            return_value=rows,
        ) as mock_chart:
            out = DataLoader().fetch(["00700"], "2024-01-01", "2024-01-31")

        assert set(out) == {"00700"}
        assert mock_chart.call_args.args[0] == "00700.HK"
