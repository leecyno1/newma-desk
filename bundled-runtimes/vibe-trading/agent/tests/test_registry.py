...
class TestFallbackChains:
    ...
    def test_chains_ordered_by_ip_ban_risk(self) -> None:
        """Equity chains keep their reviewed market-specific source order."""
        assert FALLBACK_CHAINS["a_share"] == [
            "tencent", "mootdx", "eastmoney", "baostock", "akshare", "tushare", "local",
        ]
        assert FALLBACK_CHAINS["us_equity"] == ["sina", "yahoo"]
        assert FALLBACK_CHAINS["hk_equity"] == ["yahoo"]

    def test_overseas_equities_never_use_tushare_or_yfinance(self) -> None:
        forbidden = {"tushare", "yfinance"}
        assert "tushare" in FALLBACK_CHAINS["a_share"]
        assert forbidden.isdisjoint(FALLBACK_CHAINS["us_equity"])
        assert forbidden.isdisjoint(FALLBACK_CHAINS["hk_equity"])

    def test_us_equity_prefers_sina_before_yahoo(self) -> None:
        """US bars follow the global-stock-data Skill source order."""
        chain = FALLBACK_CHAINS["us_equity"]
        assert chain[:2] == ["sina", "yahoo"]
