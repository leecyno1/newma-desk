...
def test_mcp_server_version_lookup_does_not_import_cli_package() -> None:
    """MCP startup must not initialize the large legacy CLI for one string."""
    source = (AGENT_DIR / "mcp_server.py").read_text(encoding="utf-8")

    assert "cli._version" not in source
