...
def test_query_fundamentals_keeps_latest_visible_revision() -> None:
    provider = TushareFundamentalProvider(api=_RestatementApi())

    before = provider.query_fundamentals(
        "income", ["000001.SZ"], as_of="2024-05-01", fields=["revenue"]
    )
    after = provider.query_fundamentals(
        "income", ["000001.SZ"], as_of="2024-05-31", fields=["revenue"]
    )

    assert before.set_index("end_date").loc["20231231", "revenue"] == 100.0
    assert after.set_index("end_date").loc["20231231", "revenue"] == 95.0
    assert not after.duplicated(["ts_code", "end_date"]).any()


def test_enrichment_never_regresses_to_an_older_period() -> None:
    dates = pd.to_datetime(["2024-02-01", "2024-05-01", "2024-05-16"])
    bars = pd.DataFrame({"close": [10.0, 11.0, 12.0]}, index=dates)
    provider = TushareFundamentalProvider(api=_RestatementApi())
    ...
    assert result.loc[pd.Timestamp("2024-02-01"), "income_revenue"] == 100.0
    assert result.loc[pd.Timestamp("2024-05-01"), "income_end_date"] == "20240331"
    assert result.loc[pd.Timestamp("2024-05-16"), "income_end_date"] == "20240331"
    assert result.loc[pd.Timestamp("2024-05-16"), "income_revenue"] == 30.0
