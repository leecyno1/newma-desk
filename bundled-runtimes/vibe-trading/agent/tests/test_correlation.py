...
def test_spearman_correlation_handles_ties_without_scipy() -> None:
    left = np.array([1.0, 1.0, 2.0, 3.0])
    right = np.array([4.0, 4.0, 2.0, 1.0])

    assert _spearman_correlation(left, right) == pytest.approx(-1.0)
...
class TestRollingCorrelationMatrix:
    ...
    def test_does_not_forward_fill_missing_closes(self):
        ...
        gap_closes = [close / 2.0 for close in peer_closes]
        gap_closes[2] = np.nan
        labels, matrix = _rolling_correlation_matrix(
            {
        ...
        assert matrix[labels.index("GAP")][labels.index("PEER")] == pytest.approx(1.0)
