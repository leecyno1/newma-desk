...
_INTEGRATED_DESK_RUNTIME = os.getenv(
...
# Desk owns conversations, channels and scheduling in integrated mode.  Keep
# their route modules out of the import graph entirely so a read-only Trading
# runtime does not pay their startup and dependency cost.
if not _INTEGRATED_DESK_RUNTIME:
...
# Keep Starlette's default lifespan so route modules can append their own
# shutdown hooks (for example QuickRunManager) without a second lifecycle
# abstraction. ``add_event_handler`` preserves that composition while avoiding
# FastAPI's deprecated decorator API.
app.router.add_event_handler("startup", _run_startup_preflight)
app.router.add_event_handler("shutdown", _stop_scheduled_research_on_shutdown)
...
if not _INTEGRATED_DESK_RUNTIME:
    # Desk Agent owns conversations, goals and long-running tasks.
...
register_settings_routes(app, include_llm=not _INTEGRATED_DESK_RUNTIME)
