...
def _spearman_correlation(left: np.ndarray, right: np.ndarray) -> float:
    ...
    Keeping the small implementation here removes SciPy from the integrated
    runtime while preserving the standard tie-handling semantics.
    """
...
def _rolling_correlation_matrix(
    ...
    for code in codes:
        ...
        # A missing close is a non-trading observation, not a flat return.
        # Disable pandas' legacy forward-fill default so halted sessions do
        # not become fabricated 0% returns in the paired sample.
        rets = ts.pct_change(fill_method=None).dropna()
        rets.name = code
        returns_frames.append(rets)

    # Align all series to a common index (inner join)
    aligned = pd.concat(returns_frames, axis=1, sort=True).dropna()
    if aligned.empty:
        ranges = {
