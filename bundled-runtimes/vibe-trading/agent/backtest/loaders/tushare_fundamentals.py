...
class TushareFundamentalProvider:
    ...
    def query_fundamentals(
        ...
        *,
        ...
        """Return the latest visible revision for each financial period."""
        result = self._query_pit_cut(
            table,
            codes,
            as_of=as_of,
            periods=periods,
        ...
        result = result.assign(
            _effective_pit_date=self._effective_pit_dates(schema, result)
        )
        result = result.sort_values("_effective_pit_date", kind="stable")
        result = result.drop_duplicates(["ts_code", "end_date"], keep="last")
        result = result.drop(columns="_effective_pit_date")
    ...
    def _query_pit_cut(
        ...
        *,
        ...
        """Return all filings visible by ``as_of`` before revision deduplication."""
        schema = self.describe_table(table)
        ...
        pit_dates = self._effective_pit_dates(schema, result)
    ...
    @staticmethod
    def _effective_pit_dates(schema: TableSchema, frame: pd.DataFrame) -> pd.Series:
        """Resolve actual disclosure dates, falling back to announcement dates."""
        pit_column = schema.point_in_time_column
        if pit_column not in frame.columns or frame[pit_column].isna().all():
            pit_column = "ann_date"
        values = frame[pit_column]
        if pit_column != "ann_date" and "ann_date" in frame.columns:
            values = values.where(values.notna(), frame["ann_date"])
        return values.map(_parse_tushare_date)
...
def enrich_price_frames_with_fundamentals(
    ...
    *,
    ...
    visible only on or after its announcement/disclosure date. Same-period
    restatements take effect on their own disclosure date, while a late filing
    for an older period cannot replace a newer period already visible.
    """
    ...
    for table, fields in fields_by_table.items():
        ...
        for code, frame in enriched.items():
            ...
            rows["_pit_date"] = provider._effective_pit_dates(schema, rows)
            rows["_period_end"] = rows["end_date"].map(_parse_tushare_date)
            rows = rows.dropna(subset=["_pit_date", "_period_end"]).sort_values(
                ["_pit_date", "_period_end"], kind="stable"
            )
            if rows.empty:
                continue

            timeline_rows: list[pd.Series] = []
            current_period: pd.Timestamp | None = None
            for _, row in rows.iterrows():
                period_end = row["_period_end"]
                if current_period is None or period_end >= current_period:
                    timeline_rows.append(row)
                    current_period = period_end
            if not timeline_rows:
                continue

            timeline = pd.DataFrame(timeline_rows).drop(columns="_period_end")
            value_columns = [
                column for column in timeline.columns if column not in {"ts_code", "_pit_date"}
            ]
