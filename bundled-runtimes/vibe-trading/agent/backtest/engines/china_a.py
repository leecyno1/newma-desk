...
class ChinaAEngine(BaseEngine):
    ...
    def can_execute(self, symbol: str, direction: int, bar: pd.Series) -> bool:
        ...
        # 3. Price limits are evaluated at the actual open fill, not by using
        # the current bar's later close.
...
def _blocked_by_limit(
    ...
    """Return whether the prospective open fill touches a daily price band."""
    band = engine.limit_band(symbol, bar, limit)
    if band is None:
        return False
    lower, upper = band
    if direction == 0:
        position_direction = position_direction or 1
        side = -position_direction
    else:
        side = direction
    fill = engine.prospective_fill_price(bar, side)
    if fill is None:
        return False
    tolerance = 1e-9 * max(abs(lower), abs(upper), 1.0)
    return fill >= upper - tolerance if side > 0 else fill <= lower + tolerance
