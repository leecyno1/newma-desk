...
class ChatLLM:
    ...
    def stream_chat(
        ...
        Returns:
            Parsed ``LLMResponse``.
        """
        if tools and _should_disable_streaming_tool_calls():
...
def _should_disable_streaming_tool_calls() -> bool:
    """Avoid SiliconFlow hangs while streaming function-call chunks."""
    cfg = get_env_config().llm
    if cfg.vibe_trading_stream_tool_calls:
        return False
    base_url = (
        cfg.openai_base_url
