...
def _build_native_deepseek(
    *,
    ...
    api_key = os.getenv(key_env or "", "") or os.getenv("OPENAI_API_KEY", "")  # env-gate: — dynamic provider key resolution
    base_url = os.getenv(base_env, "") or os.getenv("OPENAI_BASE_URL", "") or os.getenv("OPENAI_API_BASE", "")  # env-gate: — dynamic provider URL resolution
...
def _load_env_file(path: Path, *, protected_keys: set[str] | None = None) -> None:
    """Load one dotenv file while preserving process and earlier-file priority.

    Non-empty values from an earlier candidate win. Empty dotenv placeholders
    may be filled by a later project-local file, which is important when a
    user-level config exists but leaves data credentials blank.
    """
    ...
    for raw in path.read_text(encoding="utf-8").splitlines():
        ...
        current = os.environ.get(key)  # env-gate: — dotenv precedence merge
        if current in (None, ""):
            os.environ[key] = normalized


def _ensure_dotenv() -> None:
    """Load all dotenv candidates in priority order without overriding env."""
    global _dotenv_loaded
    if _dotenv_loaded:
        return
    loaded: list[Path] = []
    protected_keys = set(os.environ.copy())
    for candidate in _ENV_CANDIDATES:
        if candidate.exists():
            _load_env_file(candidate, protected_keys=protected_keys)
            loaded.append(candidate)
    if loaded:
        # Dotenv loading mutates os.environ after some CLI imports have already
        # read the singleton (for example, terminal theme initialization).
        reset_env_config()
    _dotenv_loaded = True
    ...
    logger.info(
        ...
        " + ".join(_redact_env_source(path) for path in loaded)
        if loaded
        else _redact_env_source(None),
        get_env_config().llm.langchain_provider,
        get_env_config().llm.langchain_model_name or "(unset)",
        _redact_base_url_for_log(os.getenv("OPENAI_BASE_URL") or os.getenv("OPENAI_API_BASE")),  # env-gate: — diagnostic display
    )
...
def _sync_provider_env() -> None:
    ...
    if key_env is not None:
        api_key = os.getenv(key_env, "") or os.getenv("OPENAI_API_KEY", "")  # env-gate: — dynamic provider key fallback
    else:
        api_key = os.getenv("OPENAI_API_KEY", "") or "ollama"  # env-gate: — ollama default key

    # Resolve base URL: provider-specific env → OPENAI_BASE_URL fallback
    base_url = os.getenv(base_env, "") or os.getenv("OPENAI_BASE_URL", "") or os.getenv("OPENAI_API_BASE", "")  # env-gate: — dynamic provider URL chain
...
def provider_diagnostics() -> dict[str, Any]:
    ...
    return {
        ...
        "env": {
            ...
            "OPENAI_BASE_URL": _redact_base_url_for_log(os.getenv("OPENAI_BASE_URL")),  # env-gate: — diagnostic snapshot
            "OPENAI_API_BASE": _redact_base_url_for_log(os.getenv("OPENAI_API_BASE")),  # env-gate: — diagnostic snapshot
        },
        "proxy": {
            name: _redact_proxy_url(name, os.getenv(name))  # env-gate: — proxy env iteration
