...
def _check_llm_provider() -> CheckResult:
    ...
    if any(
        ...
        for name in (
            ...
            "VIBEDESK_INTEGRATED_DOMAIN_RUNTIME",
        )
    ):
        ...
        _ensure_dotenv()
        return CheckResult(
            name="VibeDesk Agent Gateway",
            status="ready",
            message="managed by the Desk-level Agent settings",
            impact="",
        )
    ...
    base_url = os.getenv("OPENAI_BASE_URL", "") or os.getenv("OPENAI_API_BASE", "")  # env-gate: — diagnostic base URL fallback
...
def _check_global_stock_data() -> CheckResult:
    """Probe the public quote routes required by ``global-stock-data``.

    This deliberately avoids importing or querying the yfinance SDK.  The
    repository Skill defines Sina as the primary US quote route and Tencent as
    the primary HK quote route; checking those two lightweight endpoints gives
    startup diagnostics that match the actual runtime routing.
    """
    import requests
    ...
    ready: list[str] = []
    failed: list[str] = []
    for label, url, headers, marker in probes:
        try:
            response = requests.get(url, headers=headers, timeout=10)
            response.raise_for_status()
            if marker not in response.text or '=""' in response.text:
                raise ValueError("empty quote payload")
            ready.append(label)
        except Exception as exc:  # noqa: BLE001 - report each independent route
            failed.append(f"{label} {type(exc).__name__}")

    if ready:
        message = f"ready: {', '.join(ready)}"
        if failed:
            message += f"; fallback required: {', '.join(failed)}"
        return CheckResult(
            name="global-stock-data",
            status="ready",
            message=message,
            impact="" if not failed else "one overseas quote route is degraded",
        )
    return CheckResult(
        name="global-stock-data",
        status="error",
        message=f"public quote probes failed: {', '.join(failed)}",
        impact="US/HK equity data unavailable until a public route recovers",
    )
