...
class BenchRequest(BaseModel):
    ...
    strict: bool = False
    oos_split: str | None = Field(None, min_length=10, max_length=10)
    random_seeds: int = Field(5, ge=1, le=100)
    ...
    @field_validator("oos_split")
    @classmethod
    def _valid_oos_split(cls, value: str | None) -> str | None:
        if value is None:
            return None
        try:
            parsed = datetime.strptime(value, "%Y-%m-%d")
        except ValueError as exc:
            raise ValueError("oos_split must be YYYY-MM-DD") from exc
        return parsed.strftime("%Y-%m-%d")

    @model_validator(mode="after")
    def _strict_only_options(self) -> "BenchRequest":
        if not self.strict and (self.oos_split is not None or self.random_seeds != 5):
            raise ValueError("oos_split and custom random_seeds require strict=true")
        return self
...
def register_alpha_routes(
    ...
    async def get_bench_status(job_id: str) -> dict[str, Any]:
        """Return an authenticated snapshot for Desk clients that cannot use SSE headers."""
        return _job_status(ALPHA_BENCH_JOBS, job_id, _result_for_wire)
    ...
    @app.get(
        "/alpha/compare/{job_id}",
        dependencies=[Depends(require_auth)],
    )
    async def get_compare_status(job_id: str) -> dict[str, Any]:
        """Return an authenticated comparison snapshot for embedded Desk clients."""
        return _job_status(ALPHA_COMPARE_JOBS, job_id, _compare_result_for_wire)
...
def _job_status(
    ...
    """Project one in-memory job onto the same safe payload used by SSE."""
    if not _JOB_ID_RE.fullmatch(job_id or ""):
        raise HTTPException(status_code=400, detail="invalid job_id")
    with _JOBS_LOCK:
        job = jobs.get(job_id)
    ...
    return {
        ...
        "status": status_value,
        "progress": progress,
        "result": project_result(result) if isinstance(result, dict) else None,
