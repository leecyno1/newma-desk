...
from backtest.strategy_ledger import build_strategy_ledger
from src.api.quick_runs import (
    QuickRunCapacityError,
    QuickRunConflictError,
    QuickRunManager,
    QuickRunNotFoundError,
    QuickRunRequest,
    QuickRunStatus,
)
...
def _load_json_file(path: Path) -> Optional[Dict[str, Any]]:
    """Load one JSON object, degrading visibly when an artifact is corrupt."""
    try:
        if not path.exists():
            return None
        payload = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(payload, dict):
            logger.warning("Ignoring non-object JSON artifact %s", path)
            return None
        return payload
...
def _load_csv_to_dict(path: Path, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.warning("Unable to read CSV artifact %s: %s", path, exc)
        return []
...
def _build_response_from_run_dir(
    ...
    *,
    ...
    response.run_card = _load_json_file(run_card_path)
    response.strategy_ledger = build_strategy_ledger(
        run_dir,
        config=_load_json_file(run_dir / "config.json"),
        metrics=response.metrics.model_dump(mode="json") if response.metrics else None,
        run_card=response.run_card,
        state=state_data,
        request_payload=_load_json_file(run_dir / "req.json"),
        design=response.strategy_spec,
    )
    ...
    response.validation = _load_json_file(validation_path)
...
def register_runs_routes(
    ...
    quick_runs = QuickRunManager(_host_RUNS_DIR)
    app.state.quick_run_manager = quick_runs
    app.router.add_event_handler("shutdown", quick_runs.shutdown)

    # --- Routes ---

    @app.post(
        "/runs/quick",
        response_model=QuickRunStatus,
        status_code=status.HTTP_202_ACCEPTED,
        dependencies=[Depends(require_auth)],
    )
    async def create_quick_run(payload: QuickRunRequest) -> QuickRunStatus:
        """Create one fixed-template, non-Agent backtest job."""
        try:
            return quick_runs.create(payload)
        except ValidationError as exc:
            raise HTTPException(
                status_code=422,
                detail=f"invalid template params: {exc}",
            ) from exc
        except QuickRunCapacityError as exc:
            raise HTTPException(
    ...
    async def get_quick_run_status(run_id: str) -> QuickRunStatus:
        """Return the small persisted state without loading report artifacts."""
        _host_validate_path_param(run_id, "run_id")
        try:
            return quick_runs.status(run_id)
        except QuickRunNotFoundError as exc:
    ...
    async def cancel_quick_run(run_id: str) -> QuickRunStatus:
        """Cancel a queued/running quick job and terminate its child process."""
        _host_validate_path_param(run_id, "run_id")
        try:
            return await quick_runs.cancel(run_id)
        except QuickRunNotFoundError as exc:
            raise HTTPException(status_code=404, detail=f"Run {run_id} not found") from exc
        except QuickRunConflictError as exc:
    ...
    async def list_runs(limit: int = 20):
        ...
        for d in run_dirs[:limit]:
            ...
            metrics_rows = _load_csv_to_dict(metrics_file, limit=1)
            if metrics_rows:
                try:
                    metrics_row = metrics_rows[0]
                    total_return = float(metrics_row.get("total_return", 0) or 0)
                    sharpe = float(metrics_row.get("sharpe", 0) or 0)
                except (TypeError, ValueError) as exc:
                    logger.warning("Unable to parse metrics artifact %s: %s", metrics_file, exc)

            run_context = load_run_context(d)
            strategy_ledger = build_strategy_ledger(
                d,
                config=_load_json_file(d / "config.json"),
                metrics=metrics_row,
                run_card=_load_json_file(d / "run_card.json"),
                state=state_file,
                request_payload=req_data,
                design=_load_json_file(d / "design_spec.json"),
            )
