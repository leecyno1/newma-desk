...
def _sync_runtime_env(provider: LLMProviderOption, updates: Dict[str, str]) -> None:
    ...
    if provider.api_key_env:
        key_value = os.environ.get(provider.api_key_env, "")  # env-gate: dynamic provider API-key name
    ...
    base_url = os.environ.get(provider.base_url_env, "")  # env-gate: dynamic provider base-URL name
    if base_url:
...
def _register_llm_settings_routes(
    app: FastAPI,
    require_local_or_auth: AuthDep,
    require_settings_write_auth: AuthDep,
) -> None:
    """Mount the standalone LLM settings surface.

    Newma-Desk owns model selection and credentials in integrated mode, so the
    assembler can omit this Interface while retaining Trading's data-source
    settings.
    """
...
def _register_data_source_settings_routes(
    app: FastAPI,
    require_local_or_auth: AuthDep,
    require_settings_write_auth: AuthDep,
) -> None:
    """Mount the Trading-owned market-data credential settings surface."""
...
def register_settings_routes(
    ...
    *,
    ...
    """Mount settings routes, optionally excluding Desk-owned model settings."""
    ...
    if include_llm:
        _register_llm_settings_routes(
            app,
            require_local_or_auth,
            require_settings_write_auth,
        )
    _register_data_source_settings_routes(
        app,
        require_local_or_auth,
        require_settings_write_auth,
    )
