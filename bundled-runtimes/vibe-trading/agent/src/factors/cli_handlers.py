    ...
    def _run_zoo(z: str, base: int, update: Callable[[int, str], None] | None) -> None:
        ...
        if sub.get("status") == "ok":
            ...
            for key in aggregate:
                aggregate[key] += int(sub.get(key, 0) or 0)
            if not universe_meta and isinstance(sub.get("meta"), dict):
                universe_meta.update(sub["meta"])
            if sub.get("random_control") is not None:
                strict_metadata.update(
                    {
                        "random_control": bool(sub.get("random_control")),
                        "n_random_seeds": sub.get("n_random_seeds"),
                        "oos_split": sub.get("oos_split"),
                    }
                )
        else:
...
def cmd_alpha_bench(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        random_seeds = int(getattr(args, "random_seeds", 5) or 5)
        if not strict and (oos_split or random_seeds != 5):
            _err("alpha bench: --oos-split/--random-seeds require --strict")
            return 1
        if random_seeds < 1:
            _err("alpha bench: --random-seeds must be at least 1")
            return 1
        try:
            if strict:
        ...
        strict_label = (
            " [strict: same-universe random control"
            + (f", OOS after {oos_split}" if oos_split else "")
            + "]"
            if strict
            else ""
        )
        _print(
            f"[bold]Bench:[/bold] {n_target} alphas x {args.universe} x {args.period}{strict_label}"
            if _console
            else f"Bench: {n_target} alphas x {args.universe} x {args.period}{strict_label}"
        )
        _print(
            ...
            envelope.update(
                {
                    "strict": True,
                    "random_control": True,
                    "random_seeds": result.get("n_random_seeds", random_seeds),
                    "oos_split": result.get("oos_split"),
                    "confirmed_alive": result.get("confirmed_alive", 0),
                    "train_only": result.get("train_only", 0),
                    "reversed_strict": result.get("reversed_strict", 0),
...
def add_subparser(subparsers: Any) -> argparse.ArgumentParser:
    ...
    p_bench.add_argument(
        "--strict",
        action="store_true",
        help="Require a same-universe random control before an alpha is confirmed",
    )
    p_bench.add_argument(
        "--oos-split",
        metavar="YYYY-MM-DD",
        default=None,
        help="Strict mode only: first day of the out-of-sample period",
    )
    p_bench.add_argument(
        "--random-seeds",
        type=int,
        default=5,
        help="Strict mode only: number of random controls per alpha (default: 5)",
    )
    p_bench.add_argument(
        "--yes",
        action="store_true",
