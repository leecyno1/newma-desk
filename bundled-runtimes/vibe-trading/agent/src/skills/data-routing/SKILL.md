...
|--------|---------|----------------|---------|-------|
| tushare | A-shares, US, HK, funds, futures, macro | Yes (`TUSHARE_TOKEN`) | China network | tushare |
...
- **A-shares**: tencent / mootdx (never banned) > eastmoney / baostock /
  akshare > tushare (`TUSHARE_TOKEN`).
- **US stocks**: sina > yahoo > stooq > tushare (`TUSHARE_TOKEN`) > eastmoney >
  yfinance > tiingo / fmp / finnhub / alphavantage (key-gated).
- **HK stocks**: yahoo > tushare (`TUSHARE_TOKEN`) > futu > eastmoney >
  yfinance > akshare.
- **Crypto**: okx (single exchange) > ccxt (multi-exchange).
- **Futures / macro / forex**: tushare > akshare.

Execution rules:
- Never hard-code API keys or tokens in scripts. Read `TUSHARE_TOKEN` from the environment.
- Prefer `config.json` + `code/signal_engine.py` + the built-in `backtest` tool;
  it validates strategy code and performs source fallback.
- For CSI 300 constituent backtests, set `"universe": "csi300"` and use
  `"source": "auto"` or `"tushare"`. The runner expands constituents with
  Tushare `index_weight`.
- When a primary source returns partial data or raises a rate-limit error, the
  runner retains successful symbols and retries only missing symbols through
  the same-market chain.
...
- With `TUSHARE_TOKEN` configured, Tushare is the preferred authenticated fallback
  before rate-limit-prone A-share, US, and HK aggregators.
