...
The project has a built-in yfinance DataLoader (`backtest/loaders/yfinance_loader.py`), retained for explicit requests, crypto fallback, and legacy compatibility. For US/HK OHLCV, `source: "auto"` follows the repository `global-stock-data` contract instead: US uses Sina then Yahoo Chart, while HK uses Yahoo Chart. yfinance is not the default overseas-equity route.
