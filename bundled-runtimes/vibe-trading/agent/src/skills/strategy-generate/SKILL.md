...
4. **Run backtest**: call the `backtest` tool (built into the engine; no need to write `run_backtest.py`)
5. **Evaluate results**: read `artifacts/metrics.csv` and judge by the review criteria
6. **Iterative fixing**: if results are poor, modify with `edit_file` → run `backtest` → re-evaluate

**You only need to write `signal_engine.py` and `config.json`. The `backtest` tool automatically validates the strategy code, handles data loading/fallback, and executes the backtest.**
...
- US stocks: uppercase letters + `.US`, such as `AAPL.US` (auto routes to Sina, then Yahoo Chart)
- Hong Kong stocks: digits + `.HK`, such as `700.HK` (auto routes to Yahoo Chart)
...
| `^[A-Z]+\.US$` | US stocks | sina | - |
| `^\d{3,5}\.HK$` | Hong Kong stocks | yahoo | - |
...
- `source`: `"auto"` is recommended. Use a concrete registered source only when the user asks; consult the `data-routing` skill for the current source list and fallback order.
...
- `universe`: optional universe shortcut. Use `"csi300"` for full CSI 300 constituent backtests; the runner expands constituents through Tushare `index_weight`, so `codes` may be omitted or left empty.
