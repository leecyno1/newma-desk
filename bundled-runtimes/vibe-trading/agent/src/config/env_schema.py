...
class LLMConfig(_EnvBase):
    ...
    vibe_trading_stream_tool_calls: EnvBool = Field(
        alias="VIBE_TRADING_STREAM_TOOL_CALLS", default=False,
    )
    openai_base_url: str = Field(alias="OPENAI_BASE_URL", default="")
    openai_api_base: str = Field(alias="OPENAI_API_BASE", default="")
    deepseek_base_url: str = Field(alias="DEEPSEEK_BASE_URL", default="")
...
class DataConfig(_EnvBase):
    ...
    tushare_rate_limit_retries: int = Field(alias="TUSHARE_RATE_LIMIT_RETRIES", default=3)
    tushare_rate_limit_sleep_seconds: float = Field(
        alias="TUSHARE_RATE_LIMIT_SLEEP_SECONDS", default=15.0,
    )
    tushare_batch_size: int = Field(alias="TUSHARE_BATCH_SIZE", default=10)
...
class AgentTuningConfig(_EnvBase):
    ...
    vibe_trading_auto_finalize_backtest: EnvBool = Field(
        alias="VIBE_TRADING_AUTO_FINALIZE_BACKTEST", default=True,
    )
    vibe_trading_agent_timeout_seconds: int = Field(
        alias="VIBE_TRADING_AGENT_TIMEOUT_SECONDS", default=180,
    )
