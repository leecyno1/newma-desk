...
def _auto_finalize_backtest() -> bool:
    """Return whether successful backtests produce an immediate final answer."""
    override = _override("AUTO_FINALIZE_BACKTEST")
    if override is not None:
        return bool(override)
    return get_env_config().agent_tuning.vibe_trading_auto_finalize_backtest
...
def _contains_cjk(text: str) -> bool:
    """Return whether text contains common CJK ideographs."""
...
def _format_metric_value(name: str, value: Any) -> str:
    """Format common run-card metrics for a compact final response."""
    if value is None or value == "":
        return "n/a"
    number = _coerce_float(value)
    if name == "trade_count":
        return str(int(number)) if number is not None else str(value)
    if number is None:
        return str(value)
    if name in {
        "total_return",
...
def _load_csv_metrics(path: Path) -> dict[str, Any]:
    if not path.exists():
        return {}
    try:
    ...
    if set(rows[0]) >= {"metric", "value"}:
        return {str(row["metric"]): row.get("value") for row in rows if row.get("metric")}
    return dict(rows[0])


def _backtest_auto_summary(run_dir: Path, *, prefer_chinese: bool = False) -> str:
    """Build a deterministic answer after a successful built-in backtest."""
    card: dict[str, Any] = {}
    ...
    if not metrics:
        metrics = _load_csv_metrics(run_dir / "artifacts" / "metrics.csv")

    backtest = card.get("backtest") or {}
    codes = backtest.get("codes") or []
    codes_text = codes if isinstance(codes, str) else ", ".join(map(str, codes))
    codes_text = codes_text or "n/a"
    start_date = backtest.get("start_date") or "n/a"
    end_date = backtest.get("end_date") or "n/a"
    sources = card.get("data_sources") or []
    source_text = ", ".join(map(str, sources)) or str(backtest.get("source") or "n/a")
    metric_order = [
        "total_return",
        "annual_return",
        "sharpe",
    ...
    available = [(name, metrics[name]) for name in metric_order if name in metrics]
    ...
    if prefer_chinese:
        ...
        lines.extend(
            f"| {labels.get(name, name)} | {_format_metric_value(name, value)} |"
    ...
    lines.extend(
        f"| {name} | {_format_metric_value(name, value)} |" for name, value in available
    )
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._terminal_tool_summary: str = ""
        self._prefer_chinese: bool = False
    ...
    def run(self, user_message: str, history: Optional[List[Dict[str, Any]]] = None, session_id: str = "") -> Dict[str, Any]:
        ...
        self._terminal_tool_summary = ""
        self._prefer_chinese = _contains_cjk(user_message)
        ...
        try:
            while iteration < self.max_iterations:
                ...
                if self._terminal_tool_summary:
                    final_content = self._terminal_tool_summary
                    trace.write_text_entry(
                        {
                            "type": "answer",
                            "iter": current_iter,
                            "source": "backtest_auto_summary",
                        },
                        field="content",
                        value=final_content,
                        offload_kind=f"backtest-auto-summary-{current_iter}",
                    )
                    react_trace.append({"type": "answer", "content": final_content[:500]})
                    break
    ...
    def _finalize_tool_result(
        ...
        if (
            _auto_finalize_backtest()
            and tc.name == "backtest"
            and success
            and not self._terminal_tool_summary
        ):
            self._capture_backtest_auto_summary(result, trace, iteration)

    def _capture_backtest_auto_summary(
        self,
        result: str,
        trace: TraceWriter,
        iteration: int,
    ) -> None:
        """Prepare a final answer immediately after a successful backtest."""
        try:
            payload = json.loads(result)
        except (TypeError, json.JSONDecodeError):
            return
        ...
        run_dir_value = payload.get("run_dir") or self.memory.run_dir
        if not run_dir_value:
            return
        ...
        self._terminal_tool_summary = _backtest_auto_summary(
            run_dir,
            prefer_chinese=self._prefer_chinese,
        )
        trace.write(
            {
                "type": "backtest_auto_summary_ready",
                "iter": iteration,
                "run_dir": str(run_dir),
            }
        )
