...
1. `load_skill("strategy-generate")` and `load_skill("data-routing")` — read the SignalEngine and source-routing contracts
2. `write_file("config.json", ...)` — source, codes, dates, parameters. For a CSI 300 constituent backtest, set `"universe": "csi300"` and use `"source": "auto"` or `"tushare"`; the runner expands constituents through Tushare. If the strategy is expected to produce ≥10 trades, include `"validation": {{"monte_carlo": {{"n_simulations": 1000}}}}` in config.json for Monte Carlo testing
...
6. Do NOT write run_backtest.py or large ad hoc market-data scripts. The built-in engine validates strategy code and performs same-market source fallback.
