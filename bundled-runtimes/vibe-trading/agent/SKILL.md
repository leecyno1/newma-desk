...
|---------|-----------|------|
| HK/US equities & crypto | None | Always free (global-stock-data public-source routing + OKX) |
...
- **HK/US equities** via the repository `global-stock-data` contract: US bars use Sina → Yahoo Chart; HK bars use Yahoo Chart; yfinance is legacy fallback only (free, no API key)
