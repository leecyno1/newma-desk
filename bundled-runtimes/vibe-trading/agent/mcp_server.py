...
Zero API key required for HK/US/crypto research markets (global-stock-data
public routes, OKX, and AKShare are free). Trading connector tools are profile-scoped and require the
...
def backtest(run_dir: str) -> str:
    ...
    - "sina": US equities, the global-stock-data default daily-bar route
    - "yahoo": US fallback and HK default daily-bar route
    - "yfinance": explicit legacy fallback for HK/US equities
...
def factor_analysis(
    ...
    Args:
        ...
        source: Data source ("tushare", "sina", "yahoo", "yfinance", "auto").
...
def get_market_data(
    ...
    Supported sources:
    - "sina": US equities (global-stock-data default, e.g. AAPL.US)
    - "yahoo": US fallback and HK default (e.g. AAPL.US, 700.HK)
    - "yfinance": explicit legacy fallback for HK/US equities
    ...
    Args:
        ...
        start_date: Start date (YYYY-MM-DD).
        end_date: End date (YYYY-MM-DD).
        source: Data source ("auto", "sina", "yahoo", "yfinance", "okx", "tushare", "baostock", "tencent", "akshare", "ccxt").
