...
"""
Scan long-cycle significance on the long-history annual panel (year index).

This is for "长周期用长面板" research:
- Data: data/indicator_panel_annual_long_history_year.parquet (1800-2024)
- Targets: 200m (~16.7y), 100m (~8.3y)

Method:
- Preprocess to a contiguous window (first..last), interpolate small gaps.
...
def _infer_value_type(col: str) -> str:
    ...
    if "YOY" in c:
        return "rate_yoy"
    # Keep unemployment in levels (it is already bounded and cyclical).
    if "UNEMPLOY" in c:
        return "level"
    if c.endswith("_PCT") or "YIELD" in c or "IR_LONG" in c or "IR_SHORT" in c or "BANK_RATE" in c:
        return "rate_level"
    # Default: treat as level-like (GDP/CPI/indices/population).
    return "level"
...
def prepare_regular_series(s: pd.Series, *, min_points: int = 80, max_missing_frac: float = 0.2) -> pd.Series | None:
    ...
    if float(w.isna().mean()) > max_missing_frac:
        return None
    # year-index: use linear interpolation.
...
def transform_for_long_cycle(s: pd.Series, value_type: str, *, hp_lamb: float = 100.0) -> pd.Series | None:
    ...
    # HP filter removes very-low-frequency drift; keep cycle component.
    try:
...
def main() -> None:
    ...
    out_path = root / "output" / "cycle_bandpower_scores_annual_long_history_long.csv"
