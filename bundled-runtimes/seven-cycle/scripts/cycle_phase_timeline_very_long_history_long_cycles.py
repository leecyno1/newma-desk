...
"""
Build composite phase timelines for long cycles (200m/100m) from very-long-history
...
- output/cycle_phase_timeline_very_long_history_long.png
- output/cycle_phase_timeline_very_long_history_long.md
"""
...
def build_cycle_composite(
    ...
    *,
    ...
    sub["n_points"] = pd.to_numeric(sub.get("n_points"), errors="coerce")
    sub = sub.dropna(subset=["bandpower_ratio"])
    sub = sub.sort_values(["bandpower_ratio", "n_points"], ascending=[False, False]).head(top_n).reset_index(drop=True)
...
def main() -> None:
    ...
    comps_dir = out_dir / "very_long_history_long_cycles"
    ...
    out_parquet = out_dir / "cycle_phase_timeline_very_long_history_long.parquet"
    ...
    fig.suptitle("Very-long-history composite long-cycle timeline (annual)", y=0.995)
    fig.tight_layout()

    out_png = out_dir / "cycle_phase_timeline_very_long_history_long.png"
    fig.savefig(out_png, dpi=150)
    plt.close(fig)

    lines: list[str] = []
    lines.append("# Very-long-history long-cycle phase timeline (annual)")
    lines.append("")
    ...
    out_md = out_dir / "cycle_phase_timeline_very_long_history_long.md"
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
