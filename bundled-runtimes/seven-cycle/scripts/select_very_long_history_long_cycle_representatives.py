...
using the *very* long-history annual panel (year index).
...
- output/cycle_representative_indicators_very_long_history_long.md
"""
...
CYCLES = [
    CycleSpec(
        200,
        "长周期(≈16-18y, 200m)",
        min_points=200,  # very long panel: require longer for stability
        top_k=25,
        keywords=("GDP", "GDPPC", "CPI", "HOUSE", "SHARE", "YIELD", "CREDIT", "MONEY", "DEBT"),
    ),
    CycleSpec(
        100,
        "中长周期(≈7-9y, 100m)",
        min_points=160,
        top_k=25,
        keywords=("GDP", "CPI", "UNEMP", "RATE", "YIELD", "SHARE", "CREDIT", "MONEY"),
    ),
]
...
def ensure_keywords_included(
    selected: pd.DataFrame, ranked: pd.DataFrame, keywords: tuple[str, ...], max_extra: int = 12
) -> pd.DataFrame:
    if not keywords:
        return selected
...
def main() -> None:
    ...
    scores_path = root / "output" / "cycle_bandpower_scores_annual_very_long_history_long.csv"
    ...
    report_lines.append("# Very-long-history long-cycle representatives (200m/100m)")
    report_lines.append("")
    ...
    for cyc in CYCLES:
        ...
        top = df.head(max(cyc.top_k * 16, 200)).copy()
        ...
        show = dedup[cols].head(60).copy() if not dedup.empty else pd.DataFrame(columns=cols)
        report_lines.append("```")
    ...
    out_path = root / "output" / "cycle_representative_indicators_very_long_history_long.csv"
    out.to_csv(out_path, index=False)

    md_path = root / "output" / "cycle_representative_indicators_very_long_history_long.md"
    md_path.write_text("\n".join(report_lines) + "\n", encoding="utf-8")
