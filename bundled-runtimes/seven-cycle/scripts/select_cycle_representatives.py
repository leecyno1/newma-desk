...
"""
Select representative indicator groups for each target cycle length.
...
- output/cycle_representative_indicators.md
"""
...
CYCLES: list[CycleSpec] = [
    CycleSpec(200, "长周期(≈16-18y)", "A", min_points=40, top_k=15, keywords=("地产", "房地产", "房", "credit", "rate")),
    CycleSpec(100, "中周期(≈7-9y)", "A", min_points=40, top_k=15, keywords=("投资", "资本", "制造", "equipment", "capex")),
    CycleSpec(42, "库存周期(≈3-4y)", "M", min_points=180, top_k=20, keywords=("库存", "原材料库存", "产成品库存", "inventory")),
    CycleSpec(20, "短周期(≈1.5-2y)", "M", min_points=180, top_k=20, keywords=("信用", "社融", "贷款", "shibor", "hibor", "policy", "liquidity")),
    CycleSpec(12, "季节性(≈1y)", "M", min_points=180, top_k=20, keywords=("季节", "PMI", "CPI", "PPI")),
]
...
def ensure_keywords_included(
    selected: pd.DataFrame, ranked: pd.DataFrame, keywords: tuple[str, ...], max_extra: int = 8
) -> pd.DataFrame:
    if not keywords:
        return selected
    if selected.empty:
...
def main() -> None:
    ...
    map_cols = ["id", "name", "universe_category", "primary_source", "backend", "base_freq", "value_type", "panel_main_column"]
    mapping_small = mapping[map_cols].drop_duplicates(subset=["id"])

    all_selected: list[pd.DataFrame] = []
    report_lines: list[str] = []
    report_lines.append("# Cycle Representative Indicators")
    report_lines.append("")
    ...
    for cyc in CYCLES:
        ...
        top = df.head(max(cyc.top_k * 8, 80)).copy()
        ...
        report_lines.append(
            to_markdown_table(
                dedup,
                cols=["id", "name", "universe_category", "value_type", "panel_main_column", "n_points", "start", "end", "bandpower_ratio"],
                max_rows=40,
            )
        )
        report_lines.append("")
    ...
    if all_selected:
        out = pd.concat(all_selected, ignore_index=True)
    else:
        out = pd.DataFrame()

    out_path = out_dir / "cycle_representative_indicators.csv"
    out.to_csv(out_path, index=False)

    md_path = out_dir / "cycle_representative_indicators.md"
    md_path.write_text("\n".join(report_lines) + "\n", encoding="utf-8")
