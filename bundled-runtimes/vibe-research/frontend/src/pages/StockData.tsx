...
import { EquityResearchFramework } from "@/components/equity/EquityResearchFramework";
import { EquityPeerComparison } from "@/components/equity/EquityPeerComparison";
import {
  ...
  type EquityResearchSnapshot, type GlobalStock,
  type EquityResearchHistoryItem,
} from "@/lib/api";
import { cn } from "@/lib/utils";
import {
  loadEquityResearchHistory,
  persistEquityResearchHistory,
...
import { conceptIdsForLabels } from "@/lib/wiki";

interface StockDataSnapshot {
  code: string;
  val: Valuation | null;
  reports: Report[];
  news: NewsItem[];
  ...
  researchFailureHistory: EquityResearchHistoryItem[];
}

function emptyStockDataSnapshot(code: string): StockDataSnapshot {
  return {
    code,
    val: null,
    reports: [],
    ...
    gstock: null,
    research: null,
    researchFailureHistory: [],
  };
}

function hasStockData(snapshot: StockDataSnapshot) {
  return Boolean(
    snapshot.val || snapshot.gstock || snapshot.research || snapshot.reports.length ||
    snapshot.news.length || snapshot.anns.length || snapshot.fin,
  );
}
...
export function StockData() {
  ...
  const [research, setResearch] = useState<EquityResearchSnapshot | null>(null);
  const [researchLoading, setResearchLoading] = useState(false);
  const [researchError, setResearchError] = useState<string | null>(null);
  const [researchFailureHistory, setResearchFailureHistory] = useState<EquityResearchHistoryItem[]>([]);
  const runIdRef = useRef(0);
  const activeCodeRef = useRef("");
  const snapshotRef = useRef<StockDataSnapshot>(emptyStockDataSnapshot(""));
  const contextRef = useRef<VibeDeskPageContext>({
    view: { id: "stock-research", title: "个股数据" },
    visibleBlocks: [],
    selection: {},
    filters: {},
    data: { source: "vibe-research-stock-data", freshness: "unknown" },
    actions: [],
    tasks: [],
  });
  ...
  const execute = useCallback(async (
    ...
    setCode(c);
    setLinkedName(requestedName);
    if (shouldEmit) {
      const market = /^\d{6}$/.test(c) ? "CN" : /^\d{5}$/.test(c) ? "HK" : "US";
    ...
    const cache = createVibeDeskSnapshotCache<StockDataSnapshot>(`stock-data:${c}`, 1, 2 * 1024 * 1024);
    const cached = cache.read()?.value;
    const resourceChanged = activeCodeRef.current !== c;
    activeCodeRef.current = c;
    const rid = ++runIdRef.current;
    const applySnapshot = (snapshot: StockDataSnapshot) => {
      snapshotRef.current = snapshot;
      setVal(snapshot.val); setReports(snapshot.reports); setNews(snapshot.news); setPctl(snapshot.pctl); setFin(snapshot.fin); setAnns(snapshot.anns);
      setDepNote(snapshot.depNote); setMargin(snapshot.margin); setBlockT(snapshot.blockT); setHolders(snapshot.holders); setDividend(snapshot.dividend);
      setFundFlow(snapshot.fundFlow); setDt(snapshot.dt); setLockup(snapshot.lockup); setBlocks(snapshot.blocks); setHotCon(snapshot.hotCon); setQa(snapshot.qa);
      setGStock(snapshot.gstock); setResearch(snapshot.research); setResearchFailureHistory(snapshot.researchFailureHistory);
    };
    if (resourceChanged) applySnapshot(cached ?? emptyStockDataSnapshot(c));
    else if (cached && !hasStockData(snapshotRef.current)) applySnapshot(cached);
    const commitSnapshot = (patch: Partial<StockDataSnapshot>) => {
      if (rid !== runIdRef.current) return;
      snapshotRef.current = { ...snapshotRef.current, ...patch, code: c };
      cache.write(snapshotRef.current);
    };
    setLoading(true); setErr(null); setResearchError(null); setResearchLoading(true);

    api.equityResearch(c)
      .then(async (snapshot) => {
        if (rid !== runIdRef.current) return;
        setResearch(snapshot);
        commitSnapshot({ research: snapshot });
        const hydrated = await persistEquityResearchHistory(snapshot);
        if (rid === runIdRef.current) {
          setResearch(hydrated);
          commitSnapshot({ research: hydrated });
        }
      })
      .catch(async (error) => {
        if (rid !== runIdRef.current) return;
        setResearchError(error instanceof ApiError ? error.message : "研究框架暂不可用");
        const history = await loadEquityResearchHistory(c);
        if (rid === runIdRef.current) {
          setResearchFailureHistory(history);
          commitSnapshot({ researchFailureHistory: history });
        }
      })
      .finally(() => {
        if (rid === runIdRef.current) setResearchLoading(false);
      });
    ...
    if (!/^\d{6}$/.test(c)) {
      try {
        ...
        if (rid === runIdRef.current) {
          setGStock(g);
          commitSnapshot({ gstock: g });
        }
      } catch (e) {
        if (rid === runIdRef.current) setErr(hasStockData(snapshotRef.current)
          ? "更新失败，当前为上次数据"
          : e instanceof ApiError ? e.message : "查询失败");
      } finally {
    ...
    const ok = <K extends keyof StockDataSnapshot>(key: K, set: (value: StockDataSnapshot[K]) => void) =>
      (value: StockDataSnapshot[K]) => {
        if (rid !== runIdRef.current) return;
        set(value);
        commitSnapshot({ [key]: value } as Pick<StockDataSnapshot, K>);
      };
    ...
    try {
      ...
      setFin(f);
      setAnns(a);
      commitSnapshot({ val: v, reports: r, pctl: p, fin: f, anns: a });
      try {
        const n = await api.news(c);
        if (rid === runIdRef.current) {
          setNews(n);
          commitSnapshot({ news: n, depNote: null });
        }
      } catch (e) {
        if (rid === runIdRef.current && e instanceof ApiError && e.status === 501) {
          setDepNote(e.message);
          commitSnapshot({ depNote: e.message });
        }
      }
    } catch (e) {
      if (rid !== runIdRef.current) return;
      setErr(hasStockData(snapshotRef.current)
        ? "更新失败，当前为上次数据"
        : e instanceof ApiError ? e.message : "查询失败");
    } finally {
      if (rid === runIdRef.current) setLoading(false);
  ...
  const run = () => void execute(code);

  const selectedMarket = val
    ? "CN"
    : gstock?.market === "HK"
      ? "HK"
      : gstock?.market === "US"
        ...
        : /^\d{5}$/.test(code)
          ? "HK"
          : "US";
  const selectedName = val?.name || gstock?.name || linkedName || code;
  const conceptIds = conceptIdsForLabels([
    ...(blocks?.concept_tags ?? []),
    ...hotCon.map((item) => item.concept),
  ]);

  contextRef.current = {
    view: { id: "stock-research", title: "个股数据" },
    visibleBlocks: [
      { id: "stock-search", type: "security-search", title: "证券查询" },
      ...(val || gstock
        ? [{ id: "stock-overview", type: "security-overview", title: "个股数据总览" }]
        : []),
      ...(research
        ? [{ id: "equity-research-framework", type: "evidence-ledger", title: "跨市场研究框架" }]
        : []),
    ],
    selection: code
      ? {
    ...
    data: {
      ...
      source: gstock
        ? (gstock.data_sources || gstock.quote.sources || ["global-stock-data"]).join(" -> ")
        : "vibe-research-stock-data",
      freshness: val || gstock ? "fresh" : loading ? "unknown" : "stale",
      summary: {
        loading,
        error: err || "",
        ...(val
        ...
        ...(gstock
          ? {
              quote: {
                symbol: gstock.code,
                name: gstock.name,
                market: gstock.market,
                price: gstock.quote.price,
                changePct: gstock.quote.change_pct,
                marketCap: gstock.quote.mcap,
              },
            }
          : {}),
        ...(research
          ? {
              researchFramework: {
                frameworkVersion: research.frameworkVersion,
                coverage: research.coverage,
                sections: research.sections,
                analytics: research.analytics,
                scorecard: research.scorecard,
                comparisonProfile: research.comparisonProfile,
                derivedEvidence: research.derivedEvidence,
                workflow: research.workflow,
                reportHistory: research.reportHistory,
                evidenceLedger: research.evidenceLedger,
                gaps: research.gaps,
              },
            }
          : {}),
        ...(researchError
          ? {
              researchFailure: researchError,
              previousResearch: researchFailureHistory.slice(0, 3),
            }
          : {}),
      },
    },
    ...
    ...(code ? {
      wiki: {
        primarySubject: {
          type: "security",
          canonicalId: `security:${selectedMarket}:${code}`,
        ...
        relatedSubjects: [],
        conceptIds,
        intent: "equity.research",
        snapshotId: `stock-research:${selectedMarket}:${code}:${research?.generatedAt ?? "current"}`,
      },
    } : {}),
    tasks: research?.workflow
      ? [{
          id: research.workflow.task.id,
          status: research.workflow.task.status,
          actionId: "research.equity-snapshot",
        }]
      : researchLoading
        ? [{ id: `equity-research:${code}`, status: "running", actionId: "research.equity-snapshot" }]
        : [],
  };

  useEffect(() => {
    const unregisterContext = registerVibeDeskContextProvider(() => contextRef.current);
    const unsubscribeConfig = subscribeVibeDeskConfig(() => {
      if (!activeCodeRef.current) return;
      const currentCode = activeCodeRef.current;
      activeCodeRef.current = "";
      void execute(currentCode, false);
    });
    const unsubscribeEvent = subscribeVibeDeskEvent((event) => {
      if (event.event !== "security.selected") return;
      const symbol = event.payload.symbol;
      if (typeof symbol !== "string") return;
      void execute(symbol, false);
    });
    const unregisterHandoff = registerVibeDeskHandoffHandler((handoff) => {
      if (
        handoff.subject.type !== "security" ||
        !handoff.subject.symbol ||
        !handoff.subject.market
      ) {
        throw new Error("个股研究只支持股票 Wiki 对象");
      }
      void execute(
        handoff.subject.symbol,
        false,
  ...
  }, [code, linkedName, loading, err, val, gstock, research, researchLoading, researchError, researchFailureHistory, reports, anns, news, blocks, hotCon]);
  ...
  const researchPanels = (
    <>
      ...
      {researchError && !research && (
        <GlassCard className="mb-4 border-warning/30 bg-warning/5">
          <p className="flex items-center gap-2 text-xs font-medium text-warning">
            <AlertCircle className="h-3.5 w-3.5" /> 研究流程未完成
          </p>
          <p className="mt-1.5 text-[11px] leading-5 text-muted-foreground">
            {researchError}。行情页面仍可使用；重新查询后会再次尝试，不会用空数据覆盖最近一次有效研究记录。
          </p>
          {researchFailureHistory[0] && (
            <p className="mt-1.5 border-t border-warning/15 pt-1.5 text-[10px] text-muted-foreground">
              Desk 中最近记录：{new Date(researchFailureHistory[0].createdAt).toLocaleString("zh-CN", { hour12: false })}
              {` · 质量 ${researchFailureHistory[0].qualityScore}/100 · 覆盖 ${Math.round(researchFailureHistory[0].coverageRatio * 100)}%`}
            </p>
          )}
        </GlassCard>
      )}
      {research && <EquityResearchFramework snapshot={research} />}
      {research && <EquityPeerComparison currentSymbol={research.identity.symbol} />}
    </>
  );
    ...
    <div>
      ...
      <div className="mb-5 flex gap-2">
        <input
          ...
          className="w-80 rounded-lg border border-border bg-card/80 px-3 py-2 text-sm outline-none placeholder:text-muted-foreground/70 focus:border-primary/50"
        />
        <button
          onClick={run}
          disabled={loading}
      ...
      {/* 美股 / 港股视图（global-stock-data Skill 多源回退） */}
      {gstock && (
        <>
          ...
          <p className="mb-4 text-xs text-muted-foreground/60">
            美股 / 港股数据直接使用 <a href="https://github.com/simonlin1212/global-stock-data" target="_blank" rel="noreferrer" className="hover:text-primary">global-stock-data</a> Skill
            （{(gstock.data_sources || gstock.quote.sources || []).join(" → ") || "公开数据源"}）· 金额为原生币种 · 仅客观数据，不含买卖建议。
          </p>
          {researchPanels}
        </>
      )}
      ...
      {!val && !gstock && !err && !loading && (
        <GlassCard>
          <div className="py-10 text-center text-sm text-muted-foreground">
            ...
            <span className="text-xs text-muted-foreground/60">数据来自公开源（腾讯行情 / 东财研报 / akshare）；Newma-Trading 不预置任何标的、不做推荐。</span>
          </div>
        </GlassCard>
      )}
