...
def test_snapshot_routes_return_latest_history_and_immutable_version(
    ...
    with TestClient(create_app(settings)) as client:
        latest = client.get("/api/mods/market-daily/snapshot")
        history = client.get("/api/mods/market-daily/snapshots")
        immutable = client.get(
            f"/api/mods/market-daily/snapshots/{snapshot.id}"
        )
        legacy_latest = client.get("/api/modules/market-daily/snapshot")
    ...
    assert history.json() == [
        {
            ...
            "url": f"/api/mods/market-daily/snapshots/{snapshot.id}",
        }
    ]
    ...
    assert legacy_latest.json() == latest.json()
...
def test_latest_snapshot_returns_404_before_first_success(tmp_path: Path) -> None:
    ...
    with TestClient(create_app(settings)) as client:
        response = client.get("/api/mods/market-daily/snapshot")
